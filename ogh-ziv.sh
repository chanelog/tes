...
ssh_add_user() {
    ...
    # Buat system user (no home, no login shell untuk keamanan)
    useradd -M -s /bin/false -e "$ue" "$un" &>/dev/null
    echo "${un}:${up}" | chpasswd &>/dev/null
    # Izinkan login via SSH (override shell supaya bisa login)
    usermod -s /bin/bash "$un" &>/dev/null
    ...
    printf  "  ${DIM} Host/IP   :${NC}  ${Y}%s${NC}\n" "$domain"
    printf  "  ${DIM} IP Publik :${NC}  ${Y}%s${NC}\n" "$ip_pub"
    echo -e "  ${A1}${_DASH}${NC}"
    printf  "  ${DIM} SSH TCP   :${NC}  ${W}22, 442, 109${NC}\n"
    printf  "  ${DIM} SSH WS    :${NC}  ${W}80, 8080${NC}\n"
    printf  "  ${DIM} SSH WSS   :${NC}  ${W}443 path /ssh${NC}\n"
    printf  "  ${DIM} SSH SSL   :${NC}  ${W}465, 777${NC}\n"
...
ssh_info_user() {
    ...
    printf  "  ${DIM} IP Publik :${NC}  ${Y}%s${NC}\n" "$ip_pub"
    echo -e "  ${A1}${_DASH}${NC}"
    echo -e "  ${DIM} SSH TCP OpenSSH  : ${W}22${NC}"
    echo -e "  ${DIM} SSH TCP Dropbear : ${W}442, 109${NC}"
    echo -e "  ${DIM} SSH WebSocket    : ${W}80, 8080${NC}"
    echo -e "  ${DIM} SSH WSS (SSL WS) : ${W}443 path /ssh${NC}"
    echo -e "  ${DIM} SSH SSL/TLS      : ${W}465, 777${NC}"
    echo -e "  ${DIM} Dropbear SSL     : ${W}1443${NC}"
    echo -e "  ${A1}${_DASH}${NC}"
    printf  "  ${DIM} Expired   :${NC}  ${Y}%s${NC}\n" "$e"
    printf  "  ${DIM} Kuota     :${NC}  ${W}%s${NC}\n" "$ql"
    printf  "  ${DIM} Catatan   :${NC}  ${DIM}%s${NC}\n" "$note"
...
xray_add_user() {
    ...
    case "$proto" in
    vmess)
        printf  "  ${DIM} TCP Port  :${NC}  ${W}%s${NC}\n" "$XRAY_VMESS_TCP"
        printf  "  ${DIM} WS Port   :${NC}  ${W}%s${NC}  ${DIM}path: /vmess-ws${NC}\n" "$XRAY_VMESS_WS"
        printf  "  ${DIM} gRPC Port :${NC}  ${W}%s${NC}  ${DIM}svc: vmess-grpc${NC}\n" "$XRAY_VMESS_GRPC"
        printf  "  ${DIM} HTTPS-WS  :${NC}  ${W}8443${NC}  ${DIM}path: /vmess-ws (TLS)${NC}\n"
        ;;
    vless)
        printf  "  ${DIM} TCP Port  :${NC}  ${W}%s${NC}\n" "$XRAY_VLESS_TCP"
        printf  "  ${DIM} WS Port   :${NC}  ${W}%s${NC}  ${DIM}path: /vless-ws${NC}\n" "$XRAY_VLESS_WS"
        printf  "  ${DIM} gRPC Port :${NC}  ${W}%s${NC}  ${DIM}svc: vless-grpc${NC}\n" "$XRAY_VLESS_GRPC"
        printf  "  ${DIM} HTTPS-WS  :${NC}  ${W}8443${NC}  ${DIM}path: /vless-ws (TLS)${NC}\n"
        ;;
    trojan)
        printf  "  ${DIM} TCP Port  :${NC}  ${W}%s${NC}\n" "$XRAY_TROJAN_TCP"
        printf  "  ${DIM} WS Port   :${NC}  ${W}%s${NC}  ${DIM}path: /trojan-ws${NC}\n" "$XRAY_TROJAN_WS"
        printf  "  ${DIM} HTTPS-WS  :${NC}  ${W}8443${NC}  ${DIM}path: /trojan-ws (TLS)${NC}\n"
        ;;
    shadowsocks)
        printf  "  ${DIM} SS Port   :${NC}  ${W}%s${NC}\n" "$XRAY_SS_PORT"
        printf  "  ${DIM} SS Method :${NC}  ${W}%s${NC}\n" "$XRAY_SS_METHOD"
        printf  "  ${DIM} SS Pass   :${NC}  ${A3}%s${NC}\n" "$ss_pass"
        inf "Catatan: Shadowsocks menggunakan 1 password bersama untuk semua akun."
        ;;
    esac
    echo -e "  ${A1}${_DASH}${NC}"
    printf  "  ${DIM} Expired   :${NC}  ${Y}%s${NC}\n" "$ue"
...
xray_info_user() {
    ...
    case "$proto" in
    vmess)
        echo -e "  ${DIM} VMess TCP  port ${W}${XRAY_VMESS_TCP}${NC}"
        echo -e "  ${DIM} VMess WS   port ${W}${XRAY_VMESS_WS}${NC}  ${DIM}path /vmess-ws${NC}"
        echo -e "  ${DIM} VMess gRPC port ${W}${XRAY_VMESS_GRPC}${NC}  ${DIM}svc vmess-grpc${NC}"
        echo -e "  ${DIM} VMess WS+TLS    ${W}8443${NC}  ${DIM}path /vmess-ws${NC}"
        ;;
    vless)
        echo -e "  ${DIM} VLESS TCP  port ${W}${XRAY_VLESS_TCP}${NC}"
        echo -e "  ${DIM} VLESS WS   port ${W}${XRAY_VLESS_WS}${NC}  ${DIM}path /vless-ws${NC}"
        echo -e "  ${DIM} VLESS gRPC port ${W}${XRAY_VLESS_GRPC}${NC}  ${DIM}svc vless-grpc${NC}"
        echo -e "  ${DIM} VLESS WS+TLS    ${W}8443${NC}  ${DIM}path /vless-ws${NC}"
        ;;
    trojan)
        echo -e "  ${DIM} Trojan TCP  port ${W}${XRAY_TROJAN_TCP}${NC}"
        echo -e "  ${DIM} Trojan WS   port ${W}${XRAY_TROJAN_WS}${NC}  ${DIM}path /trojan-ws${NC}"
        echo -e "  ${DIM} Trojan WS+TLS    ${W}8443${NC}  ${DIM}path /trojan-ws${NC}"
        ;;
    shadowsocks)
        echo -e "  ${DIM} SS Port    :${NC}  ${W}${XRAY_SS_PORT}${NC}"
        echo -e "  ${DIM} SS Method  :${NC}  ${W}${XRAY_SS_METHOD}${NC}"
        echo -e "  ${DIM} SS Pass    :${NC}  ${A3}${ss_pass}${NC}"
        ;;
    esac
    echo -e "  ${A1}${_DASH}${NC}"
    printf  "  ${DIM} Expired   :${NC}  ${Y}%s${NC}\n" "$e"
...
menu_xray() {
    while true; do
        ...
        systemctl is-active --quiet xray 2>/dev/null \
            && xray_svc_stat="${LG}● RUNNING${NC}" || xray_svc_stat="${LR}● STOPPED${NC}"
        _top
        _btn "  ${IT}${AL}  📡  V2RAY/XRAY ALL PROTOCOL${NC}"
        _sep
        printf "  ${DIM}Xray Service :${NC} %b    ${DIM}Total Akun:${NC} ${Y}%s${NC}\n" \
            "$xray_svc_stat" "$(xray_total_user)"
        _sep
        _btn "  ${A2}[1]${NC}  🚀  Install V2Ray/Xray"
        _sep; _btn "  ${A2}[2]${NC}  ➕  Tambah Akun (VMess/VLESS/Trojan/SS)"
        _sep; _btn "  ${A2}[3]${NC}  📋  List Semua Akun Xray"
        _sep; _btn "  ${A2}[4]${NC}  🔍  Detail Akun Xray"
        ...
        case ${ch,,} in
            ...
            9) xray_port_info ;;
            r) systemctl restart xray; sleep 1
               systemctl is-active --quiet xray && ok "Xray direstart!" || err "Gagal restart!"; pause ;;
            0) break ;; *) warn "Pilihan tidak valid!"; sleep 1 ;;
        esac
    done
}
