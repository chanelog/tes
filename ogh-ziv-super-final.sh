...
# ===== ADDED: ALL SSH PROTOCOL =====
install_ssh_all() {
    apt update -y
    apt install -y openssh-server dropbear stunnel4 nginx curl wget
...
EOF
    ...
    openssl req -new -x509 -days 3650 -nodes -out /etc/stunnel/stunnel.pem -keyout /etc/stunnel/stunnel.pem -subj "/C=ID/ST=Jakarta/O=VPN/CN=localhost"
...
# ====== ADDED FINAL BOSS + SSH MANAGER ======

ssh_add() {
read -p "Username: " user
read -p "Password: " pass
read -p "Hari aktif: " days
...
install_zivpn_bin() {
wget -O /usr/local/bin/zivpn-bin https://github.com/fauzanihanipah/ziv-udp/releases/download/udp-zivpn/udp-zivpn-linux-amd64
...
menu_install_bin() {
echo "1 Zivpn | 2 BadVPN"
read -p "Pilih: " x
case $x in
1) install_zivpn_bin ;;
2) install_badvpn_bin ;;
esac
}

backup_data() {
mkdir -p /root/backup
tar -czvf /root/backup/vpn.tar.gz /etc/zivpn
}

auto_repair() {
for svc in zivpn ssh dropbear stunnel4 nginx badvpn; do
...
menu_final_boss() {
echo "1 Backup | 2 Repair | 3 Fail2Ban"
read -p "Pilih: " x
case $x in
1) backup_data ;;
2) auto_repair ;;
